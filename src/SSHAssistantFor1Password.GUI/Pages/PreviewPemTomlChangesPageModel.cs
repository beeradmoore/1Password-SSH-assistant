...
public partial class PreviewPemTomlChangesPageModel : ObservableObject
{
    ...
    public async Task GenerateOutputAsync()
    {
        ...
        //stringBuilder.AppendLine("IMPORTANT: SSH Assistant for 1Password will append the below configs even if the new changes are already present in their respective config files. If you run an export on the same items multiple times you may need to manually repair your config files.");
